...
function prepareRequestData() {
    const oldPersonalID = document.getElementById('old-personal-id').value;
    const newPersonalID = document.getElementById('new-personal-id').value;
    const password = document.getElementById('pwd').value;

    return {
        old_personal_id: oldPersonalID,
...
async function verifyAccess() {
    ...
    try {
        ...
        const response = await fetch('/master/verify-switch-signing-access', {
            method: 'POST',
            headers: {
