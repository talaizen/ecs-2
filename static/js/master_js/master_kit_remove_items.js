$(function() {
    let kitId = $('#kitId').val();
    let apiUrl = `/collections-data/kit_remove_items/${kitId}`
    $('#collectionTable').DataTable({
        "ajax": {
            "url": apiUrl, // API endpoint in your FastAPI app that returns collection data
        ...
        "columns": [
            {
                ...
                "render": function (data, type, row) {
                    return `<input type="checkbox" class="item-checkbox" data-kit-item-id="${data}">`;
                },
                "orderable": false
            },
            { "data": "name" },
...
$('#itemsForm').on('submit', async function(e) {
    ...
    try {
        const response = await fetch('/master/remove_kit_item', {
            method: 'POST',
            headers: {
