...
async def get_switch_requests(request: Request, mongo_db: MongoDB = Depends(get_mongo_db)):
    try:
        user: User = await get_current_client_user(request, mongo_db)
    except (ValueError, HTTPException):
        return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
    ...
    async for document in await mongo_db.get_switch_requests_by_old_signer_pid(user.personal_id):
...
async def get_approve_switch_requests(request: Request, mongo_db: MongoDB = Depends(get_mongo_db)):
    ...
    async for document in await mongo_db.get_switch_requests_by_new_signer_pid(user.personal_id):
        signing_info = await mongo_db.get_signing_item_by_object_id(document.get("signing_id"))
        item_info = await mongo_db.get_inventory_item_by_object_id(
            signing_info.get("item_id")
        )
