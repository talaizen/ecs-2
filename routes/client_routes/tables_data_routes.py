...
async def get_switch_requests(request: Request, mongo_db: MongoDB = Depends(get_mongo_db)):
    ...
    data = []
    status_mapping = {1: "signer approval required", 2: "Mami approval required"}

    async for document in await mongo_db.get_switch_requests_by_personal_id(user.personal_id):
